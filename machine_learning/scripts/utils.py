...
def make_dendrograms(matrix, res, selected_col, genus):
    """Create dendrogram for genus-specific clustering, with error handling for degenerate inputs"""
    try:
...
def make_cluster_df(linkage_matrix, idx, selected_col, genus):
    """Create cluster analysis dataframe for genus, with error handling for degenerate inputs"""
...
def train_lr_final_model(all_predictions, bin_num, fixed_threshold=0.5):
    ...
    bin_df = prepare_bin_df(all_predictions, "stacking", num_bins=bin_num, refs_=True)
...
def train_lr_and_predict(all_predictions, methods=["stacking", "cumsum", "cumsum_reverse"]):
    ...
    for method_name in methods:
        bin_df = prepare_bin_df(all_predictions, method_name, num_bins=10, refs_=True)
        X = bin_df.drop(columns=["ground_truth"])
        y = bin_df["ground_truth"]
        model_lr = LogisticRegression(max_iter=1000)
        model_lr.fit(X, y)
...
def train_lr_and_predict_binned_test(all_predictions, nums: list):
    ...
    for num in nums:
        bin_df = prepare_bin_df(all_predictions, "stacking", num_bins=num, refs_=True)
        X = bin_df.drop(columns=["ground_truth"])
        y = bin_df["ground_truth"]
...
def prepare_bin_df(all_predictions, method_name, num_bins=10, refs_=False):
    """
    ...
    refs_ (bool): Whether ground truth is available and should be attached to the output.

    Returns:
    pd.DataFrame: The bin_df DataFrame with binned probabilities, and ground truth if refs_ is True.
    """
    ...
    if refs_:
        # Create a ground truth mapper and attach it
...
def predict_contigs(predictions, mc, idx, mc_name, refs_: bool, num_bins: int = 10):

    if refs_:
        bin_df = prepare_bin_df(predictions, mc_name, num_bins=num_bins, refs_=True)
        ...
        bin_df = prepare_bin_df(predictions, mc_name, num_bins=num_bins)
        X_test = bin_df
        final_predictions = mc.predict_proba(X_test)[:, 1]
        final_predictions_df = pd.DataFrame(
...
def add_info_basic(
    ...
    if missing_keys:
        print(f"Error: The following ORF names are missing in the 'orf_name' column of input_df: {list(missing_keys)[0:10]}")
        sys.exit(1)
