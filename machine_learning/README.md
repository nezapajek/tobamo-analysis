...
## **5.3 Testing with Viral Sequences Outside Virgaviridae Example**

Example of testing the model with divergent viral sequences (outside
*Virgaviridae*) to validate specificity:
...
- `results/model_selection/` — empty pending the reproducibility-bug fix
  re-run (see repo root notes / next session); the bug itself
  (missing `random_state` on non-RandomForest classifiers in
  `model_selection()`) is already fixed in `scripts/train_model_pipeline.py`.

Also excluded (superseded/exploratory notebooks, not part of the citable
pipeline): `notebooks/B_model_cv_evaluation.ipynb`,
`notebooks/makeviz_scrapbook.ipynb`,
`notebooks/results_comparison_different_sampling.ipynb`,
`notebooks/check_false_positives.ipynb` (depended on a gold-standard
TP/TN/FP/FN labeling file that lived only in a dropped scratch folder),
`notebooks/E_comparison_different_approaches.ipynb`,
`notebooks/F_rank-k_stats.ipynb`,
`notebooks/C_model_feature_importance.ipynb`, and
`visualizations/supp_othr.ipynb`. The current, maintained
comparison/visualization notebooks live under `visualizations/`.
